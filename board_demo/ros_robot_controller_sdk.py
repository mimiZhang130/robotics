...
class PacketFunction(enum.IntEnum):
    # 可通过串口实现的控制功能(some control functions can be implemented through the serial port)
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED控制(LED control)
    PACKET_FUNC_BUZZER = 2  # 蜂鸣器控制(buzzer control)
    PACKET_FUNC_MOTOR = 3  # 电机控制(motor control)
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(control PWM servos, which are numbered 1 to 4 from inside to outside on the board)
...
class Board:
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符(The subcommand for setting SSID is 0x01, and the second byte is the length of the string, which includes the '\0' string terminator)
